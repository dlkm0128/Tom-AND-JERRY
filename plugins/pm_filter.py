...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(❤️‍🔥script.ALRT_TXT.format(❤️‍🔥query.from_user.first_name), show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer(❤️‍🔥script.OLD_ALRT_TXT.format(❤️‍🔥query.from_user.first_name),show_alert=True)
        return
