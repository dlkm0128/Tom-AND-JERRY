...
async def next_page(bot, query):
    ...
    if settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"❤️‍🔥[{get_size(file.file_size)}❤️‍🔥] ❤️‍🔥{file.file_name}❤️‍🔥", callback_data=f'❤️‍🔥files#{file.file_id}'
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"❤️‍🔥{file.file_name}❤️‍🔥", callback_data=f'❤️‍🔥files#{file.file_id}'
                ),
                InlineKeyboardButton(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    InlineKeyboardButton('⤬ Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ ⤬', url=f'CAACAgUAAxkBAAEFwNNjFHRA5DSb5XRkJ6K3JvAAAeGb41EAAgsAA8EkMTG1Uu8kMP4p0CkE')
                ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"❤️‍🔥[{get_size(file.file_size)}] ❤️‍🔥{file.file_name}❤️‍🔥", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
